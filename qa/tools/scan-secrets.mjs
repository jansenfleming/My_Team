...
// Owner: Security / QA Engineer. CI calls this exact path: `node qa/tools/scan-secrets.mjs`.
//
// Usage:
//   node qa/tools/scan-secrets.mjs                 scan the working tree (tracked + untracked, respecting .gitignore)
//   node qa/tools/scan-secrets.mjs --history       scan every added line in git history (all refs)
//   node qa/tools/scan-secrets.mjs --path <p> ...  scan specific files or directories instead (for example apps/web/dist)
//   node qa/tools/scan-secrets.mjs --json          machine-readable output
//   node qa/tools/scan-secrets.mjs --root <dir>    repository root (default: git top level of the cwd, else the cwd)
//   node qa/tools/scan-secrets.mjs --fail-on-warn  treat warnings as failures
//
...
// entry with a written reason to qa/tools/scan-secrets.allow.json.
//
// Severities:
...
function loadAllowlist(root) {
  const candidates = [join(root, "qa", "tools", "scan-secrets.allow.json"), join(HERE, "scan-secrets.allow.json")];

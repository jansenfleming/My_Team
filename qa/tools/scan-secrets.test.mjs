// Self-test for scan-secrets.mjs. Uses only node:test. Run: node --test "tools/*.test.mjs" (from qa/) or `npm test -w @site/qa`.
...
test("inline allow marker and allowlist file (with reason) suppress findings", () => {
  ...
  try {
    ...
    mkdirSync(join(d, "qa", "tools"), { recursive: true });
    writeFileSync(join(d, "qa", "tools", "scan-secrets.allow.json"), JSON.stringify({ entries: [{ path: "b.ts", rule: "github-token", reason: "test double" }] }));
...
test("allowlist entry without a reason is rejected (exit 2)", () => {
  ...
  try {
    mkdirSync(join(d, "qa", "tools"), { recursive: true });
    writeFileSync(join(d, "qa", "tools", "scan-secrets.allow.json"), JSON.stringify({ entries: [{ path: "x" }] }));

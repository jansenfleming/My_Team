...
### Branch and merge flow (simulated pull request)
...
6. Architect reviews the diff against the contract, ownership map, and definition of done, then merges `--no-ff` into `main`. **No merge without a QA gate PASS for code branches.** Docs-only branches (`docs/design/**`) need Architect review only. QA's own branches (`qa/**`) need Architect review and a green run of the suite.
7. Architect updates `docs/architecture/board.md` (status) after each merge. Board status is the source of truth; teammates report status by message, they do not edit the board.
...
### Never
Push, fetch/rebase/rewrite `main`, touch the `origin` remote, open a real PR, deploy, publish, call external services, commit secrets, invent facts about the owner, probe any host other than `localhost`.

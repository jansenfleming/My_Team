...
| E1 | engineer | Web shell and routing | none | review (changes requested) |
...
2. **Now that `feat/design-brand` is merged:** D2, D3, D4 can all start (independent of
   ...
   **E1 status (2026-09-24):** reviewed, one small required fix requested (a missing
   test for modified-click/`Link` behavior — everything else approved); see
   `docs/architecture/reviews/feat-web-shell.md`. E2 should keep waiting on the real
   E1 merge, not start against the unmerged branch, per the worktree working
   agreement — expected to be a quick turnaround once the Engineer adds the test.

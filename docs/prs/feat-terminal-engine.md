...
Author: frontend-engineer. Branch: `feat/terminal-engine` (the board says `feat/web-terminal-engine`; the lead named this one). Base: `main` at 916c5fc, then `git merge main` (twice; latest 6dc6df5, includes the vite 7 pin) before the final run. Worktree: `.worktrees/web`.
...
- **Dev server (found a bug on `main`, now fixed there).** After B1 merged, `npm run dev` returned HTTP 500 for every module (`Missing field moduleType, Plugin: builtin:vite-react-refresh-wrapper`): `packages/shared` pulled `vite@8.3.0` in at the root and the hoisted `@vitejs/plugin-react@5` used it against `apps/web`'s `vite@7.3.6`. Tests and build were unaffected. I reported it; the Architect pinned `vite ^7.3.6` at the root (`chore/pin-vite7`, merged as #9). After `git merge main` and `npm install` on this branch, `npm run dev -w @site/web` starts clean (log has no errors) and `/`, `/src/main.tsx`, `/src/App.tsx`, `/src/terminal/Terminal.tsx`, `/src/terminal/useTerminal.ts`, `/src/terminal/terminal.css`, `/src/content/site.ts`, `/src/commands/index.ts` all return HTTP 200. Tests, typecheck, lint and build were re-run after that merge with the same results as above (77/77, exit 0, JS 72.54 kB gzip).

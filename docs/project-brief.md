...
- **Pull requests are real from wave 2 on** (`gh` is installed and authenticated; the owner said to use real PRs). Flow: owner works on a feature branch and writes `docs/prs/<branch-slug>.md`; QA gates it (code branches); the Architect reviews and approves but **does not merge locally**. The Architect then messages the lead "ready to merge: <branch>". The lead alone pushes the branch, opens the PR (body = the PR file plus a link to the gate report), merges it with `gh pr merge --merge`, and updates local `main`. Agents never run `git push` or `gh`. Wave 1 branches were merged locally before this rule.
